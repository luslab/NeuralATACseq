#/*==========================================================================#*/
#' ## Plot Shh loci - invivo data (Figure S2J)
...
promoter.region <- GRanges(seqnames = 'chr5', ranges = IRanges(28438975, 28477817))
...
promoterInter.df <- promoter.inter %>% as.data.frame %>% tbl_df() %>%
  filter(seqnames1 == seqnames2) %>%
  filter((Cell.Tissue %in% c('Cortex', 'Cerebellum', 'ESC', 'NPC', 'NSC',
