#/*==========================================================================#*/
#' ## Plot Olig2 loci - invivo data (Figure S2K)
#+ chunk_regionPlots_ShhInvivo, cache=T, echo=F, warning=F, message=F
#/*==========================================================================#*/
...
bw.path <- '/Users/steinhs/projects/atac_vicki/results/atac_invivo//bw/'
bw.files <- atacR::listFiles(bw.path, pattern = 'fpm.bw')
bw.files <- bw.files[grep(bw.files, pattern = '(D0|D5)')]

# Get promoter region for gene
promoter.region <- promoters(genes(gencode.txdb), upstream = up.dist, downstream = down.dist)
promoter.region <- promoter.region[grep(names(promoter.region), pattern = gene.id)]
...
gg.tpmBar <- tpm.ofi %>%
  ...
  filter(grepl(cond, pattern = 'D5(A|SC)')) %>%
...
regions.promoter <- subsetByOverlaps(regions, promoter.region)

# Find interactions overlapping with the promoter region
#promoter.inter <- subsetInteractionsByGene(interactions, gene.ofi)
...
promoter.inter <- subsetInteractionsByRegion(promoter.inter, promoter.region)
...
gg.loci <- plot_grid(gg.gene, gg.tracks, gg.inter, nrow = 3, rel_heights = c(1.5, 6, 2), align = 'v', axis = 'lr')
gg.exp <- plot_grid(NULL, gg.tpmBar, legend.inter, nrow = 3, rel_heights = c(1.5, 6, 2))
